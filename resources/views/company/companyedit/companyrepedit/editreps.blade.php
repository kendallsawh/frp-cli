        ...
        <div class="col-sm-12">
            ...
            <div class="picture-container">
                <div class="fileinput fileinput-new text-center" data-provides="fileinput">
                    <div class="fileinput-new thumbnail" style="width: 200px; height: 150px; border-radius: 50%;">
                        <img src="{{session()->has('old_avatar'.$i)? asset('/storage/temp/'.session('old_avatar'.$i)) : ($rep->avatar? $rep->avatar : asset('/img/default-avatar.png'))}}" alt="Avatar">
                    </div>
                    <div class="fileinput-preview fileinput-exists avatar{{$i}} thumbnail" style="max-width: 200px; max-height: 150px; border-radius: 50%;">
                        @if($rep->avatar)<img src="{{$rep->avatar}}" >@endif
                    </div>
                    <div>
            ...
            <div class="input-group">
                ...
                <div id="grp-app_fname{{$i}}" class="form-group{{ $errors->has('app_fname'.$i) ? ' has-error' : '' }} label-floating">
                    ...
                    <input id="app_fname{{$i}}" name="app_fname{{$i}}" type="text" class="form-control" value="{{old('app_fname'.$i)? old('app_fname'.$i) : $rep->f_name}}"/>
            ...
            <div class="input-group">
                ...
                <div id="grp-app_sname{{$i}}" class="form-group{{ $errors->has('app_sname'.$i) ? ' has-error' : '' }} label-floating">
                    ...
                    <input id="app_sname{{$i}}" name="app_sname{{$i}}" type="text" class="form-control" value="{{old('app_sname'.$i)? old('app_sname'.$i) : $rep->l_name}}"/>
            ...
            <div class="input-group">
                ...
                <div id="grp-contact{{$i}}"  class="form-group{{ $errors->has('contact'.$i) ? ' has-error' : '' }} label-floating">
                    ...
                    <input id="contact{{$i}}" name="contact{{$i}}" type="text" class="form-control contact" value="{{old('contact'.$i)? old('contact'.$i) : $rep->contact}}">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div id="grp-id_type{{$i}}"  class="form-group{{ $errors->has('id_type'.$i) ? ' has-error' : '' }} label-floating">
                            ...
                            <select id="id_type{{$i}}" name="id_type{{$i}}" class="form-control dropdown">
                                ...
                                <option value="{{$id_type->id}}" {{old('id_type'.$i)==$id_type->id ? 'selected' : ($rep->id_type_id==$id_type->id ? 'selected' : '') }}>{{$id_type->identification_type}}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div id="grp-id_num{{$i}}"  class="form-group{{ $errors->has('id_num'.$i) ? ' has-error' : '' }} label-floating">
                            ...
                            <input id="id_num{{$i}}" name="id_num{{$i}}" type="text" class="form-control" value="{{old('id_num'.$i)? old('id_num'.$i) : $rep->id_num}}">
