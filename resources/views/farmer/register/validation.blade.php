...
function validate(tab) {
    ...
    switch(tab) {
    	/*remeber for each case to return either true or false based on the value of the variable chk*/
	    case 'about':
		    ...
		    if (!$('#option2').is(':checked') && !$('#option3').is(':checked')) {
	    				chk = false;
				    	$('#grp-regtype').addClass('has-error');
		            	$('#err-regtype').html('Please select an application type.');
	    			}
		    ...
		    if (n_id) {
		        if (isNaN(n_id)) {
	            	...
	            	$('#n_id_pp_link').prop("href", individual_link);//found on individual.blade page under farmer>register folder
		   	...
		   	if(passport){
		    	if(ids.indexOf(passport) != -1){
	            	...
	            	$('#n_id_pp').removeClass('hide');
	            	$('#n_id_pp_link').prop("href", individual_link);
		    	}
		    }
	    ...
	    case 'parcels':
	    	...
	    	for (i = 0, len = num.length; i < len; i++) { 
				...
				if (app && land_type && tenure) {
		            ...
		            if( 
						...
						if (doc_optional[app][land_type][tenure].length == 1) {
							opts = true;
							//console.log(opts+ ' '+ codes);
						}

						if (app == 2 && land_type == 2 && tenure == 2) {opts = true;}
						//console.log(opts+ ' '+ codes);
	        ...
	        break;
	    case 'appointment':
	    	if (!$('.app_time').is(':checked')) {
	            //Set the Valid Flag to True if one RadioButton from the Group of RadioButtons is checked.
	            //var isValid = $("input[name=app_time]").is(":checked");
	            //alert('g');
	            chk= false;
	            //Display error message if no RadioButton is checked.
	            $('#grp-appointment-date').addClass('has-error');
		        $('#err-appointment-date').html('Please select an appointment time.');
		        /*$('#grp-appointment-date').show();*/
		        
        	}
		    //another way to check radio button selection
	    	var appoint_date = $('#appoint_date').val();
	    	if (!$('.appointment_date').is(':checked')) {
		    	chk = false;
		    	$('#grp-appointment').addClass('has-error');
            	$('#err-appointment').html('Please select a date');
            	/*$('#grp-appointment').show();*/
		    }
	    	//chk= true;
	    	if (chk) {
	        	return true;
