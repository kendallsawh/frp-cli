    ...
    $(document).on('change', '.app_time', function() {
        
        $('#err-appointment-date').html('');
    ...
    $(document).on('change', '.appointment_date', function() {
        ...
        $('#err-appointment').html('');
    });

    // clear registration type error message
    $(document).on('change', '.regtype', function() {
        var id = $(this).prop('id');
        // remove error messages

        
        $('#grp-regtype').removeClass('has-error');
        $('#err-regtype').html('');
        $('#conf-regtype').html($('#'+id+' option:selected').text());
        if ($(this).is(':checked')) {
            var slug = $(this).attr('slug'); // retrieve the value
            $('#conf-regtype').html(slug);            
        }else{
            $('#conf-regtype').html().empty();
        }
    ...
    // delete parcel
    $(document).on('click', '.btn-del-parcel', function() {
        var pane = $(this).attr('pane');
        var chk = confirm('Remove Parcel '+pane+'?');
        if (chk) {
            // add button for new parcel pane
            $('#btn-add-parcel-'+pane).addClass('hide');
            // view new parcel pane
            $('button[target="parcel-pane-1"]').click();

            // show confirm section
            $('#conf-parcel-div-'+pane).addClass('hide');
            // add parcel number to list
            $('#parcels-removed').val($('#parcels-removed').val()+pane+',');
            ...
            for (i = 0, len = num.length; i < len; i++) {
                if (num[i] == pane) {
                    num.splice(i, 1);
                    $('#parcels-added').val(num.toString());
                }
            }

            $('#btn-add-parcel').addClass('btn-success').removeClass('btn-default').tooltip();
    ...
    $(document).on('click', '.tenure', function() {
        ...
        if (app && land_type && tenure) {
            //alert(app+' app - '+land_type+' lt - '+tenure+' tenure');
            // mandatory
            //check to see if array is empty and 2nd and 3rd also
            ...
            //if array index has a value then remove hide class to show the red star
            ...
            if( typeof(doc_optional) != 'undefined' &&
                ...
                doc_optional[app][land_type][tenure].forEach(function(item, index, arr) {
                    ...
                    $('#proofs_'+parcel+'_'+item).removeClass('hide');
                    //console.log(proof_conditions[app][land_type][tenure][item]);
                    if(proof_conditions[app][land_type][tenure][item] == 1){
                        $('#proof_opt_'+parcel+'_'+item).addClass('hide');
                    }
                });
